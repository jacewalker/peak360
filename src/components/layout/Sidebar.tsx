...
const ASSESSMENTS_ITEM: NavItem = {
  label: 'Assessments',
...
// D-13: Client's "My Assessments" link points to /portal because their dashboard
// already lists their owned assessments.
const MY_ASSESSMENTS_ITEM: NavItem = {
  label: 'My Assessments',
  href: '/portal',
  icon: ASSESSMENTS_ITEM.icon,
  matchExact: false,
  matchPaths: ['/portal/assessment'],
};
...
export default function Sidebar() {
  ...
  // D-13: Role → nav-items mapping.
  // D-12: When role is undefined (useSession() still resolving), render only
  // Dashboard so privileged items (Clients, Admin) MUST NOT flash for non-admins.
  // Strict positive equality is used everywhere; negative checks are forbidden
  // because the loading state (role === undefined) would silently slip through.
  const navItems = useMemo<NavItem[]>(() => {
    if (role === 'client') {
      return [DASHBOARD_ITEM, MY_ASSESSMENTS_ITEM];
    }
    if (role === 'coach') {
    ...
    // Loading / unknown role — Dashboard only.
