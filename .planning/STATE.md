...
| 260601-r25 | TODO-001 landing polish (9 items): desktop eyebrow size, solid Live Sample teaser, sample report image in §3, reworded Process steps + "Recommended" label, deck pricing (Baseline $1,000 / Retest $750 / $500 + iMedical note + add-ons), 180×180 apple-touch-icon, dark-bg OG image, iPhone logo aspect-ratio fix, and a ContactModal → POST /api/contact (honeypot+validation) emailing info@strongbodies.com.au via SMTP2Go. Build clean; endpoint verified. Testimonials still blocked on Kevin. | 2026-06-01 | ad49c2b | [260601-r25-implement-9-actionable-items-from-todo-0](./quick/260601-r25-implement-9-actionable-items-from-todo-0/) |
| 260601-rn9 | Contact form polish: restyle the "Interested in" select to match the text inputs (appearance:none + gold caret + dark options), and add Cloudflare Turnstile (explicit dark widget gating submit, single-use reset; server verifies in /api/contact when TURNSTILE_SECRET_KEY set, skips otherwise; TEST-key fallback for dev; env vars documented). Build clean; widget + select verified in browser. | 2026-06-01 | f75e944 | [260601-rn9-fix-contact-form-select-styling-and-add-](./quick/260601-rn9-fix-contact-form-select-styling-and-add-/) |
